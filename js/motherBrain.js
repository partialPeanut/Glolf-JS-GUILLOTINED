...
function calculateStrokeResult(worldState, tl, player, dist) {
    ...
    const hole = activeHoleOnTimeline(worldState, tl)
    let sinkChance = 0
    if (dist >= player.ball.distance) sinkChance = curveExpy(player.ball.distance, 0.3, 0.8) * hole.stats.obedience * strokeType.sinkFactor * curveLoggy(0.5, 1.5, player.stats.charisma)
